...
class EventManager {
  ...
  + createEventPlace(EventPlace eventPlace) onlyRole(MASTER_ROLE)
  + setEventPlace(uint256 eventPlaceId, EventPlace eventPlace) onlyRole(MASTER_ROLE))
  + approveEventRequest(uint256 eventRequestId, Multihash eventPlaceDataCID) onlyRole(MASTER_ROLE)
  + declineEventRequest(uint256 eventRequestId) onlyRole(MASTER_ROLE)
  ...
  + submitEventRequest(EventRequest request)
...
EventManager --> EventRequest
EventManager --> EventPlace
EventManager --> Event
EventManager --> Multihash
EventManager --o TetherToken
TetherToken ..|> IERC20
Event --> Multihash
#+end_src
