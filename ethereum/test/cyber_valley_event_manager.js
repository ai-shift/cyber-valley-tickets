...
const CyberValleyEventManager = contract.fromArtifact(
...
describe("CyberValleyEventManager", () => {
  ...
  beforeEach(async () => {
    ...
    eventManager = await CyberValleyEventManager.new(
      usdtToken.address,
      DEV_TEAM_PERCENTAGE,
      MASTER_PERCENTAGE,
      { from: owner },
    ...
    // Deploy Event Ticket
    eventTicket = await CyberValleyEventTicket.new(
      "CyberValleyEventTicket",
      "CVT",
      eventManager.address,
      { from: owner },
    );
    await eventManager.setCyberValleyTicketContract(eventTicket.address, {
      from: owner,
    });

    // Grant master role
    await eventManager.grantRole(await eventManager.MASTER_ROLE(), master, {
      from: owner,
    });
    ...
    await usdtToken.approve(eventManager.address, TICKET_PRICE, {
      from: customer,
    });
  });

  describe("Roles", () => {
    it("should allow the owner to grant the master role", async () => {
      await eventManager.grantRole(await eventManager.MASTER_ROLE(), master, {
        from: owner,
      });
      (
        await eventManager.hasRole(await eventManager.MASTER_ROLE(), master)
      ).should.be.equal(true);
    });

    it("should allow the owner to grant the staff role", async () => {
      await eventManager.grantRole(await eventManager.STAFF_ROLE(), staff, {
        from: owner,
      });
      (
        await eventManager.hasRole(await eventManager.STAFF_ROLE(), staff)
      ).should.be.equal(true);
    });

    it("should revert if a non-owner tries to grant a role", async () => {
      await expectRevert(
        eventManager.grantRole(await eventManager.MASTER_ROLE(), master, {
          from: customer,
        }),
  ...
  describe("Event Place Management", () => {
    const MAX_TICKETS = new BN("100");
    const MIN_TICKETS = new BN("10");
    const MIN_PRICE = new BN("50");
    const MIN_DAYS = new BN("7");

    it("should allow the master to create an event place", async () => {
      const { logs } = await eventManager.createEventPlace(
        MAX_TICKETS,
        MIN_TICKETS,
        MIN_PRICE,
        MIN_DAYS,
    ...
    it("should revert if a non-master tries to create an event place", async () => {
      await expectRevert(
        eventManager.createEventPlace(
          MAX_TICKETS,
          MIN_TICKETS,
          MIN_PRICE,
          MIN_DAYS,
    ...
    it("should allow the master to update an event place", async () => {
      ...
      const { logs } = await eventManager.updateEventPlace(
        0,
        NEW_MAX_TICKETS,
        NEW_MIN_TICKETS,
        NEW_MIN_PRICE,
    ...
    it("should revert if a non-master tries to update an event place", async () => {
      await eventManager.createEventPlace(
        MAX_TICKETS,
        MIN_TICKETS,
        MIN_PRICE,
  ...
  describe("Event Request Management", () => {
    const MAX_TICKETS = new BN("100");
    const MIN_TICKETS = new BN("10");
    const MIN_PRICE = new BN("50");
    const MIN_DAYS = new BN("7");
    ...
    it("should allow a user to submit an event request", async () => {
      const { logs } = await eventManager.submitEventRequest(
        0,
        START_TIMESTAMP,
        TICKET_PRICE,
        { from: creator },
    ...
    it("should allow the master to approve an event request", async () => {
      await eventManager.submitEventRequest(0, START_TIMESTAMP, TICKET_PRICE, {
        from: creator,
      });

      const { logs } = await eventManager.approveEventRequest(
        0,
        EVENT_DATA_CID,
        { from: master },
      );
    ...
    it("should revert if a non-master tries to approve an event request", async () => {
      await eventManager.submitEventRequest(0, START_TIMESTAMP, TICKET_PRICE, {
        from: creator,
      });
    ...
    it("should allow the master to decline an event request", async () => {
      await eventManager.submitEventRequest(0, START_TIMESTAMP, TICKET_PRICE, {
        from: creator,
      });

      const { logs } = await eventManager.declineEventRequest(0, {
        from: master,
      });
    ...
    it("should revert if a non-master tries to decline an event request", async () => {
      await eventManager.submitEventRequest(0, START_TIMESTAMP, TICKET_PRICE, {
        from: creator,
      });
  ...
  describe("Event Management", () => {
    ...
    beforeEach(async () => {
      ...
      await eventManager.approveEventRequest(0, EVENT_DATA_CID, {
        from: master,
      });
    });

    it("should allow the master to update an event", async () => {
      const NEW_EVENT_DATA_CID =
        "QmXWATWQmAWAmegNmUjVuF4ffLYsjyKtUFxYR27pS3CcU6Z";

      const { logs } = await eventManager.updateEvent(0, NEW_EVENT_DATA_CID, {
        from: master,
      });
    ...
    it("should revert if a non-master tries to update an event", async () => {
      const NEW_EVENT_DATA_CID =
        "QmXWATWQmAWAmegNmUjVuF4ffLYsjyKtUFxYR27pS3CcU6Z";
    ...
    it("should allow the master to cancel an event", async () => {
    ...
    it("should revert if a non-master tries to cancel an event", async () => {
      await expectRevert(
    ...
    it("should allow the master to close an event", async () => {
      ...
      const devTeamCut =
        (web3.utils.toWei("1", "ether") * DEV_TEAM_PERCENTAGE) / 100;
      const masterCut =
        (web3.utils.toWei("1", "ether") * MASTER_PERCENTAGE) / 100;
      const creatorCut =
        web3.utils.toWei("1", "ether") - devTeamCut - masterCut;

      assert.approximately(
        Number(finalDevTeamBalance),
        Number(initialDevTeamBalance) + Number(devTeamCut),
        Number(web3.utils.toWei("0.001", "ether")),
      );
      assert.approximately(
        Number(finalMasterBalance),
        Number(initialMasterBalance) + Number(masterCut),
        Number(web3.utils.toWei("0.001", "ether")),
      );
      assert.approximately(
        Number(finalCreatorBalance),
        Number(initialCreatorBalance) + Number(creatorCut),
        Number(web3.utils.toWei("0.001", "ether")),
      );
    });

    it("should revert if a non-master tries to close an event", async () => {
      await expectRevert(
  ...
  describe("Ticket Management", () => {
    ...
    it("should allow staff to verify a ticket", async () => {
      await eventManager.buyTicket(0, { from: customer });
      (await eventManager.verifyTicket(0, 0, { from: staff })).should.be.equal(
        true,
      );
    });

    it("should revert if a non-staff tries to verify a ticket", async () => {
      await eventManager.buyTicket(0, { from: customer });
      await expectRevert(
