...
export const Camera: React.FC<CameraProps> = ({ open, setOpen, onShot }) => {
  ...
  const capture = useCallback(
    () => {
      ...
      const image = getImage(JSON.stringify({
        type: "image/jpeg",
    ...
    <CustomModal open={open} setOpen={setOpen}>
      <CustomModalWindow>
        <div className="flex flex-col items-center gap-3">
          <X className="h-10 w-10 cursor-pointer text-red-500 self-end" onClick={() => setOpen(false)} />
          <WebCam
            ref={webcamRef}
            audio={false}
            height={1080}
            ...
            videoConstraints={{...videoConstaints, torch: flash, facingMode: mode } as MediaTrackConstraints}
          />
          <div className="flex justify-between items-center w-full">
            <div className="h-10 w-10 bg-yellow-500"  onClick={() => setFlash(prev => !prev)}/>
            <div className="h-10 w-10 bg-green-500" onClick={capture}/>
            <div className="h-10 w-10 bg-blue-500" onClick={() => toggleMode()}/>
          </div>
        </div>
      </CustomModalWindow>
    </CustomModal>
