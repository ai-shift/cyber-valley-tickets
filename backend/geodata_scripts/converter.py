...
def resolve_normal_style(style_element, global_styles):
    """
    If the style_element is a StyleMap, resolves it to the specific 'normal' Style element.
    If it's a regular Style, returns it as is.
    """
    # Check if the element is a StyleMap
    if style_element is not None and style_element.tag == f'{{{KML_NAMESPACE}}}StyleMap':
        ...
        for pair in style_element.findall(f'./{{{KML_NAMESPACE}}}Pair'):
            ...
            if key_tag is not None and key_tag.text == 'normal':
                ...
                if style_url_tag is not None and style_url_tag.text:
                    # Recursively resolve the next level (which should be the final Style)
...
def placemark_to_json(placemark, global_styles):
    """
    ...
    if style_url_tag is not None and style_url_tag.text:
        initial_style_element = get_style_element_by_url(style_url_tag.text, global_styles)
    ...
    def get_coordinates(geometry_tag):
        ...
        return coords_tag.text.strip() if coords_tag is not None and coords_tag.text is not None else ""
    ...
    if polygon_tag is not None:
        ...
        if final_style is not None:
            ...
            if line_style_tag is not None:
                ...
                # Added None check
                line_color = color_tag.text.strip() if color_tag is not None and color_tag.text is not None else line_color
            ...
            if poly_style_tag is not None:
                ...
                # Added None check
                poly_color = color_tag.text.strip() if color_tag is not None and color_tag.text is not None else poly_color
        ...
        if final_style is not None:
            ...
            if icon_style_tag is not None:
                ...
                if icon_tag is not None:
                    ...
                    # Added None check
                    icon_url = href_tag.text.strip() if href_tag is not None and href_tag.text is not None else icon_url
...
def convert_kml_to_json(input_kml_path, output_json_path="kml_output.json"):
    ...
    global_styles = {}
    # Find Document element anywhere in the tree, which is the typical container for styles
    ...
    for style_type in ['Style', 'StyleMap']:
        ...
        for style_element in style_container.findall(f'.//{{{KML_NAMESPACE}}}{style_type}'):
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description="Convert KML Placemarks (Polygon, LineString, Point) to a single JSON array.",
    ...
    parser.add_argument(
        'input_file', 
        type=str, 
        help="Path to the input KML file (e.g., 'data.kml')."
    )
