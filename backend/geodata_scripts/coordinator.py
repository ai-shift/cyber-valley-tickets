...
def parse_coordinates(coord_string, geom_type):
    ...
    Args:
        coord_string (str): The raw coordinate string from the KML
          (space-separated, LON,LAT,ALT comma-separated).
        geom_type (str): The geometry type ('point', 'polygon', 'line').

    Returns:
        list or dict: A list of coordinate objects for lines/polygons,
        or a single object for points.
    """
    if not coord_string:
...
def transform_json_coordinates(input_data):
    ...
    for feature in input_data:
        ...
        if "coordinates" in new_feature and new_feature["type"] in (
            "polygon",
            "line",
            "point",
        ):
            ...
            new_feature["coordinates"] = parse_coordinates(
                coord_string, new_feature["type"]
            )
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Transforms raw KML coordinate strings (LON,LAT,ALT)"
            " in a JSON array into structured {lat, lng} objects."
        ),
    ...
    if not Path(args.input_json_file).exists():
    ...
    try:
        with Path(args.input_json_file).open(encoding="utf-8") as f:
